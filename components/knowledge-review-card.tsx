...
export function KnowledgeReviewCard({
  ...
  onDecision: (toolCallId: string, decision: ReviewDecision) => void
}) {
  ...
  return (
    <div className="w-full rounded-2xl border bg-popover p-4 text-sm shadow-lg">
      ...
      <div className="mt-3 flex flex-wrap gap-2">
        ...
        <Button size="sm" variant="outline" onClick={() => submit("rejected")}>
          Reject
        </Button>
      </div>
      <p className="mt-2 text-xs text-muted-foreground">
        Review does not promote or persist the item.
      </p>
    </div>
  )
}
